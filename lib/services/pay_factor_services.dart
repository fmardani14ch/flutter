...
class PAY_FACTOR_SERVICE {
  ...
  static Future<List<PAY_FACTOR_MODEL>> get_pay_factor_data() async {
    //************************json متغیری برای دریافت اطلاعات به صورت جیسان */
    var response = await httpClient.get("smv75c/data");
...
//*** یک متغیر که بهتر است هم نام کلاس ولی با نام کوچکتر باشد ارسال میشود */
  static Future<PAY_FACTOR_MODEL> add_user_factor(
    ...
    var response =
        await httpClient.post("smv75c/data", data: pay_factor_model.toJson());
    //*********** اگر شرط بر قرار بود و سرور به ما پاسخ صحیح یا قبولی داد */
    if (response.statusCode == 200 || response.statusCode == 201) {
      //******************************************* اطلاعات را ارسال می کنیم */
      await get_pay_factor_data();
...
//##############################################################################
//##############################################################################
  static Future<PAY_FACTOR_MODEL> delete_user_factor(int id) async {
